    ...
    <D as BlockDevice>::Error: core::fmt::Debug,
{
    ...
    pub async fn write<const BLOCKS: usize>(
        ...
        debug!("write(file={:?}, buffer={:?}", file, buffer);
    ...
    /// Update the entry for a file with the given raw file and get its index.
    pub async fn flush(&mut self, file: RawFile) -> Result<(), Error<D::Error>> {
        // Retrieve file info
        let file_info = (|| {
            for (_, info) in self.open_files.iter().enumerate() {
    ...
    /// Determines how many bytes are left in the current block. This can be used to fill up the current block,
    /// such that subsequent writes can be block-aligned.
    pub async fn remaining_in_block(&mut self, file: RawFile) -> Result<usize, Error<D::Error>> {
        let file_idx = self.get_file_by_id(file)?;
        let file_offset = self.open_files[file_idx].current_offset;
        let block_offset = (file_offset % Block::LEN_U32) as usize;
